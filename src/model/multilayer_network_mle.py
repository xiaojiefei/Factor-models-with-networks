...
def estimate_single_window(R_win: np.ndarray, F_win: np.ndarray,
                           ...
                           dirichlet_alpha: float = 0.0) -> Dict:
    """
    ...
    bounds = [(0, 1)] * d + [(-2.0, 2.0)] * K

    best_result = None
    best_loglik = -np.inf

    for delta_init in init_deltas:
        for rho_init_val in [-0.5, -0.1, 0.1, 0.3, 0.5]:
...
def rolling_estimation(R: np.ndarray, F: np.ndarray,
    ...
    """
    Rolling window one-step concentrated MLE estimation.

    Bonaccolto et al. (2019): jointly estimate delta and heterogeneous rho_i.
    """
    K = R.shape[1]
    d = len(W_layers_3d)
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"Rolling Window One-Step Concentrated MLE")
        print(f"{'='*60}")
    ...
    for count, ci in enumerate(eval_indices):
        ...
        for j, name in enumerate(layer_names):
            row[f'delta_{name}'] = res['delta_hat'][j]
        for i in range(K):
            row[f'lambda_{i}'] = res['lambda_hat'][i]
        ...
        if verbose and ((count + 1) % 20 == 0 or count == 0 or count == n_eval - 1):
            ...
            delta_str = " ".join(
                f"d_{name}={res['delta_hat'][j]:.3f}" for j, name in enumerate(layer_names))
            lam_mean = np.mean(res['lambda_hat'])
...
def main_rolling(config: Optional[NetworkConfig] = None):
    ...
    # Save results
    results_path = os.path.join(config.results_dir, "rolling_delta.csv")
    centrality_path = os.path.join(config.results_dir, "rolling_centrality.csv")
    ...
    plot_delta_evolution(
        ...
        save_path=os.path.join(config.results_dir, "delta_evolution.png"))
    plot_centrality_heatmap(
        centrality_df, industry_names,
        save_path=os.path.join(config.results_dir, "centrality_heatmap.png"))
    plot_top_central_industries(
        centrality_df, industry_names, top_n=5,
        save_path=os.path.join(config.results_dir, "top_central_industries.png"))
